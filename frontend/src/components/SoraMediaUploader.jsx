...
const SoraMediaUploader = () => {
    ...
    const fetchSubscriptionInfo = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log('[SoraMediaUploader] Subscription info fetched:', data);
          console.log('[SoraMediaUploader] Selected plan:', data?.selectedPlan);
          setSubscriptionInfo(data);
        } else {
          console.error('[SoraMediaUploader] Subscription fetch failed:', response.status);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (content?.mediaFile && content?.mediaType === 'video' && !formData.files) {
      console.log('🎬 SoraMediaUploader: Detected downloaded video, auto-populating form data');
      ...
      console.log('🎬 SoraMediaUploader: Auto-populated with downloaded video:', {
  ...
  const validateFile = (file) => {
    ...
    // Debug logging
    console.log('[SoraMediaUploader] File validation:', {
      fileSize: file.size,
    ...
    if (subscriptionInfo?.selectedPlan === 'starter') {
      ...
      console.log('[SoraMediaUploader] Applying Starter plan limit: 3MB');
    } else if (subscriptionInfo?.selectedPlan === 'creator') {
      maxSize = 50 * 1024 * 1024; // 50MB for Creator plan
      console.log('[SoraMediaUploader] Applying Creator plan limit: 50MB');
    } else {
      ...
      console.log('[SoraMediaUploader] Applying platform limit:', Math.floor(maxSize / (1024 * 1024)) + 'MB');
    }

    console.log('[SoraMediaUploader] Final maxSize:', Math.floor(maxSize / (1024 * 1024)) + 'MB');
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <h1 className="text-xl font-bold">Sora Media Upload</h1>
          <Link
            to="/app/sora"
      ...
      <main className="[sora-media-uploader] max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          {(formData.preview || content.mediaUrl) && (
            console.log('🎬 SoraMediaUploader Preview - content:', {
              mediaUrl: content.mediaUrl,
              mediaType: content.mediaType,
...
export default SoraMediaUploader;
