...
ARG REACT_APP_PYTHON_API_URL       # <- compat (some scripts use this name)
ARG REACT_APP_CLERK_PUBLISHABLE_KEY
ARG PUBLIC_URL="/"

# Make both names available to the build; prefer REACT_APP_AI_API, fall back to PYTHON_API_URL
...
# Container-level healthcheck (separate from ALB health check)
